...
**Puțin** is a Romanian practice app built with Go, React, TypeScript, and PostgreSQL. Anyone can practise published questions. GitHub sign-in unlocks personal practice history; admins also manage private course materials and questions. Anonymous answers are graded without saving history.
...
Practice immediately displays one random question from the five built-in questions and published course questions. Questions with no previous attempt take priority, regardless of whether earlier answers were correct. Once every question has been attempted, practice selects from the full pool. Within those priorities, selection varies formats and learning targets, avoiding three consecutive questions of the same format when alternatives exist; equally suitable questions are picked randomly. Shuffle question and Next question avoid repeating the current question when another question at the same priority is available. Signed-in learners use their saved history; anonymous practice remembers up to 200 answered question IDs only in memory for the current visit. The eight most recently shown question IDs stay in browser memory to help vary the practice; signed-in learners also use recent saved history after reloading. Publishing makes questions and their feedback available to everyone; original documents and unpublished drafts are accessible only to admins. Practice history remains private to the learner.

Practice supports typed gaps, single-choice questions, and multi-select questions with checkboxes. Single-choice and multi-select prompts can ask about grammar, vocabulary, a dialogue, or a short passage without containing a gap. Multi-select requires every correct option and no incorrect options; feedback lists the full correct set after a mistake. Saved history preserves the selected and correct options even if an admin later changes the question.

Learners can report a question with an optional note before or after answering. Reports are private to admins, who can review the question, edit it, and mark the report resolved from the Reports tab. Anonymous reports do not create accounts or practice history. Practice feedback shows the answer and explanation without lesson excerpts; source evidence remains available in the admin question editor.
...
Choose **AI · generate + Romanian review** in the generation controls to send the teaching text to the configured provider. Uploading documents does not trigger API calls. AI generation requests an explicit format mix:

| Maximum drafts | Single choice | Typed gap | Multi-select |
| --- | --- | --- | --- |
| 5 | 2 | 2 | 1 |
| 10 | 5 | 4 | 1 |
| 20 (default) | 10 | 8 | 2 |

The adapter checks source quotes, structural validity, distinct choices, and complete answer keys. Approved choice options are shuffled before saving so their position does not follow the model’s answer ordering. A separate LLM request checks Romanian grammar, spelling, diacritics, ambiguity, and explanations, independently solving every answer option. Generated questions include a skill, specific learning target, and difficulty; these guide varied practice and are editable in **Learning focus**. Skill and difficulty preferences adapt to the lesson, while format counts are enforced. A batch accepts at most two questions with the same declared skill and target.

When questions are missing or rejected, generation makes at most one targeted replacement attempt, with its own independent review. It never fills a missing format with extra questions of another format. The saved **Last AI generation** table shows requested and created counts, including any shortfall. Only reviewed questions are saved: an invalid or incomplete review discards that candidate batch, while previously approved questions can still be retained. If no questions pass, nothing is saved.

The loader stays visible through generation, review, and any replacements, within the existing two-minute limit. All calls use the configured provider, model, and effort. AI generation is selected by default when configured. The explicitly labeled local method remains offline, creates recall exercises only, and does not enforce this mix or receive an AI review. Provider errors remain redacted. Admins can correct any question later. API generation accepts up to 30 KB of usable text per request; split larger documents into lessons.
