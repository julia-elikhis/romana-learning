...
export function History(){
 ...
 {items?.map(item=><article className="panel history-item" key={item.id}><div className="step"><span className="status-tag">{item.correct?'Correct':'Keep practising'}</span><time dateTime={item.createdAt}>{new Date(item.createdAt).toLocaleString()}</time></div><h2 lang="ro">{item.prompt||'Earlier practice question'}</h2><p>Your answer: <strong lang="ro">{item.answer}</strong>{!item.correct&&<><br/>Correct answer: <strong lang="ro">{item.correctAnswer}</strong></>}</p><p>{item.explanation}</p></article>)}
 </main>
}
