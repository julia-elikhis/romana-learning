export type ExerciseKind='cloze'|'multiple_choice'|'multi_select';
export const formatNames:Record<ExerciseKind,string>={multiple_choice:'Single choice',cloze:'Typed gap',multi_select:'Select all'};
export type Exercise={id:string;kind:ExerciseKind;prompt:string;options:string[]};
export type Progress={attempts:number;correct:number;practiced:number;available:number;tracked:boolean};
export type Result={correct:boolean;answer:string;explanation:string;sourceQuote?:string;answers?:string[];saved:boolean};
...
export type HistoryItem={id:string;exerciseId:string;prompt:string;answer:string;correct:boolean;correctAnswer:string;explanation:string;createdAt:string;selections?:string[];correctSelections?:string[]};
...
export type LearningFocus={skill?:''|'grammar'|'vocabulary'|'communication'|'reading';target?:string;difficulty?:''|'easy'|'medium'|'hard'};
export type FormatCounts=Record<ExerciseKind,number>;
export type GenerationSummary={requested:FormatCounts;created:FormatCounts;shortfall:FormatCounts;refillAttempted:boolean;note?:string};
export type Draft=Exercise&LearningFocus&{answers:string[];explanation:string;sourceQuote:string;sourceLine:number;status:'draft'|'published'|'rejected'};
export type DraftEdit=LearningFocus&{kind:Draft['kind'];prompt:string;answers:string[];options:string[];explanation:string};
export type LibraryData={enabled:boolean;apiEnabled:boolean;generator:string;materials:Material[];generationMixes:Record<string,FormatCounts>};
export type MaterialData={material:Material;exercises:Draft[];generator:string;generationSummary?:GenerationSummary};
