...
func Migrate(ctx context.Context, db *sql.DB) error {
 ...
 CREATE TABLE IF NOT EXISTS attempts(id TEXT PRIMARY KEY,exercise_id TEXT NOT NULL,answer TEXT NOT NULL,correct BOOLEAN NOT NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT now());`); err != nil {
	...
	if err = tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version=7)`).Scan(&installed); err != nil {
		return err
	}
	if !installed {
		if _, err = tx.ExecContext(ctx, questionReportsMigration); err != nil {
			return err
		}
